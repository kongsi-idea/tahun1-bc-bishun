...
**状态**：✅ **已上线并在用**。`https://tahun1-bc-bishun.vercel.app`，已上架课堂点子铺。**当前 v1.3**。
**最后更新**：2026-09-11（Claude Sonnet 5）

## v1.3（2026-09-11）修多音字读错音 + 加拼音标注
老师回报：字音朗读有些字听起来是错的。**根因**：`speechSynthesis` 读孤立的单个汉字，
遇到多音字（如「长」「了」「系」）引擎会猜一个读音，常常不是课本这一课教的那个。

**修法**（跟 tahun2/tahun3-bc-bishun 同一套，三个工具一起修）：
1. **新增 `js/pinyin-data.js`**：282 字逐个配 `{py, word}` —— py 是这个字在 word 里的实际读音，
   word 是课本核对过的定音词。生成流程：`pypinyin` 算默认读音（258 字单音字，直接可信）+
   `jieba` 切课本原文语料抓真实词语核对多音字（22 字课本原文里两种读音都出现过，逐个对照
   课本原句判断这一课教哪个；2 字课本查无退通用词典）。每条都跑过校验：word 含 ch、
   `pypinyin(word)` 在该位置算出的读音等于存的 py。
2. **画面显示拼音**：三步（看一看/描一描/写一写）+ 小考都在田字格上方标"字 拼音"小标签
   （`.pinyin-tag`）。图片缺失或查无这个字时标签自动隐藏，不影响功能。
3. **朗读改读「字，词」**：听读音按钮 + 笔顺演示完自动读音，都从读孤字改成读"字，词"
   （例："了，为了" "长，长大" "系鞋带"这种单字词直接读词）。词的上下文强制引擎读对音。

⚠️ 少数字课本本身有意教「一字两读」（如「干：树干／干花」），已在 `js/pinyin-data.js` 顶部
注释和 handoff 里说明只能收录一种，若老师觉得某个字选的读音/词不合适，改
`pinyin-data.js` 对应一行 JSON 即可，不用重新生成整份。
