...
(function () {
  ...
  /* ---------- 声音（WebAudio，生成式，可静音） ---------- */
  var Sfx = (function () {
    var ctx = null, muted = localStorage.getItem("bishun_muted") === "1";
    function ac() {
    ...
    return {
      ...
      toggle: function () { muted = !muted; localStorage.setItem("bishun_muted", muted ? "1" : "0"); if (!muted) this.resume(); return muted; },
  ...
  async function fetchRemoteProgress() {
    ...
    try {
      var res = await withTimeout(supabaseClient
        .from("tahun1_bc_bishun_progress")
        .select("chars_json")
  ...
  async function pushProgress() {
    if (!HAS_DB || !student || !student.playCode) return;
    try {
      await withTimeout(supabaseClient.rpc("submit_tahun1_bc_bishun_progress", {
  ...
  function countDoneFor(pc, nm) {
    try {
      var p = JSON.parse(localStorage.getItem("bishun_prog__" + (pc || "guest") + "__" + nm)) || {};
  ...
  function startQuizFlow(ch, hintAfter, onDone) {
    writer.quiz({
      ...
      onMistake: function () { Sfx.miss(); flash("#D98236"); },
  ...
  var qTarget = $("#quizTarget"), qStamp = $("#quizStamp"), qCoach = $("#quizCoach"),
      qActions = $("#quizActions"), qTian = $("#quizTian"), qPinyinTag = $("#quizPinyinTag");
